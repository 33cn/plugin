...
func TestPbftlibbyz(t *testing.T) {
	q, chain, p2pnet, s, mem, exec, cs, wallet := initEnvPbftlibbyz()
	defer chain.Close()
	defer mem.Close()
	defer p2pnet.Close()
	defer exec.Close()
...
func initEnvPbftlibbyz() (queue.Queue, *blockchain.BlockChain, *p2p.P2p, queue.Module, *mempool.Mempool, queue.Module, queue.Module, queue.Module) {
