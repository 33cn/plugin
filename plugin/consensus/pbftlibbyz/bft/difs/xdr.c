...
xdr_statfsres(XDR *xdrs, statfsres *objp)
{
	...
	switch (objp->status) {
	case NFS_OK:
		/* RAF * must coerce values to prevent replica mismatch */
		/* RAF * else will get infinite loop as 3f+1 will disagree */
		objp->info.tsize=8192;			/* RAF */
		objp->info.bsize=1024;			/* RAF */
		objp->info.blocks=200000;		/* RAF */
		objp->info.bfree=100000;		/* RAF */
		objp->info.bavail=100000;		/* RAF */
...
// fprintf(stderr, "xdr_statfsres: returning TRUE\n"); 	/* RAF */
// fprintf(stderr, "tsize=%ld\n",objp->info.tsize); 	/* RAF */
// fprintf(stderr, "bsize=%ld\n",objp->info.bsize); 	/* RAF */
// fprintf(stderr, "blocks=%ld\n",objp->info.blocks);	/* RAF */
// fprintf(stderr, "bfree=%ld\n",objp->info.bfree); 	/* RAF */
// fprintf(stderr, "bavail=%ld\n",objp->info.bavail); 	/* RAF */
	return (TRUE);
}
...
xdr_exportnode(XDR *xdrs, exportnode *objp)
{
	...
	if (!xdr_groups(xdrs, &objp->_groups)) {
		return (FALSE);
	}
