...
func Test_ShowAddr(t *testing.T) {
	{
		...
		relayer := &Relayer4Ethereum{
			...
			maturityDegree:      cfg.EthRelayerCfg[0].EthMaturityDegree,
			fetchHeightPeriodMs: cfg.EthRelayerCfg[0].EthBlockFetchPeriod,
		}
...
func Test_DeployContrcts(t *testing.T) {
	...
	deployerAddr := crypto.PubkeyToAddress(relayer.privateKey4Ethereum.PublicKey)
	relayer.operatorInfo = &ethtxs.OperatorInfo{
		PrivateKey: relayer.privateKey4Ethereum,
