...
func GenRands(allw, w int, priv crypto.PrivKey, blockHeight int64, blockHash []byte, round, step int) (*Pos33Rands, *pb.Signature) {
	// 本轮难度：委员会票数 / (总票数 * 在线率)
	...
	data := []byte(string(blockHash) + fmt.Sprintf(":%d%d%d", blockHeight, step, round))
...
func CheckRands(addr string, allw, w int, rs *Pos33Rands, blockHeight int64, blockHash []byte, sig *pb.Signature, round, step int) error {
	size := Pos33VerifierSize
	if step == 0 {
		size = Pos33ProposerSize
	}
