...
// Query_Pos33AllTicketCount query all ticket count
func (ticket *Ticket) Query_Pos33AllTicketCount(param *pty.Pos33AllTicketCount) (types.Message, error) {
	count, err := ticket.getAllTicketCount(param.Height)
	if err != nil {
		return nil, err
	}
	return &pty.ReplyPos33AllTicketCount{Count: int64(count)}, nil
}
