...
func getHistoryAccountByRoot(localdb dbm.KV, targetRootHash string) (*zt.HistoryAccountProofInfo, error) {
	...
	for i := uint64(1); i <= proof.ProofId; i++ {
		...
		operations := transferPubDataToOps(data.PubDatas)
	...
	if historyAccts.RootHash != targetRootHash {
		return nil, errors.Wrapf(types.ErrInvalidParam, "calc root=%s,expect=%s", historyAccts.RootHash, targetRootHash)
	}
