...
func BenchmarkCopyBlockDetail(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = CopyBlockDetail(&block)
	}
}

func CopyBlockDetail(b *types.BlockDetail) *types.BlockDetail {
	return &types.BlockDetail{
		Block:          copyBlock(b.Block),
		Receipts:       copyReceiptData(b.Receipts),
		KV:             copyKV(b.KV),
		PrevStatusHash: b.PrevStatusHash,
	}
}

func copyReceiptData(b []*types.ReceiptData) []*types.ReceiptData {
	rs := make([]*types.ReceiptData, 0, len(b))
	for i := 0; i < len(b); i++ {
		rs = append(rs, &types.ReceiptData{Ty: b[i].Ty, Logs: copyReceiptLog(b[i].Logs)})
	}
	return rs
}

func copyReceiptLog(b []*types.ReceiptLog) []*types.ReceiptLog {
	rs := make([]*types.ReceiptLog, 0, len(b))
	for i := 0; i < len(b); i++ {
		rs = append(rs, &types.ReceiptLog{Ty: b[i].Ty, Log: b[i].Log})
	}
	return rs
}
...
func copyBlock(b *types.Block) *types.Block {
	return &types.Block{
		Version:    b.Version,    // int64          `protobuf:"varint,1,opt,name=version,proto3" json:"version,omitempty"`
		ParentHash: b.ParentHash, //   []byte         `protobuf:"bytes,2,opt,name=parentHash,proto3" json:"parentHash,omitempty"`
		TxHash:     b.TxHash,     //     []byte         `protobuf:"bytes,3,opt,name=txHash,proto3" json:"txHash,omitempty"`
		StateHash:  b.StateHash,  //    []byte         `protobuf:"bytes,4,opt,name=stateHash,proto3" json:"stateHash,omitempty"`
		Height:     b.Height,     //   int64          `protobuf:"varint,5,opt,name=height,proto3" json:"height,omitempty"`
		BlockTime:  b.Height,     //  int64          `protobuf:"varint,6,opt,name=blockTime,proto3" json:"blockTime,omitempty"`
		Difficulty: b.Difficulty, // ,   uint32         `protobuf:"varint,11,opt,name=difficulty,proto3" json:"difficulty,omitempty"`
		MainHash:   b.MainHash,   //    []byte         `protobuf:"bytes,12,opt,name=mainHash,proto3" json:"mainHash,omitempty"`
		MainHeight: b.MainHeight, //    int64          `protobuf:"varint,13,opt,name=mainHeight,proto3" json:"mainHeight,omitempty"`
		Signature:  copySignature(b.Signature),
		Txs:        copyTx(b.Txs), //    []*Transaction `protobuf:"bytes,7,rep,name=txs,proto3" json:"txs,omitempty"`
	}
}

func copySignature(b *types.Signature) *types.Signature {
	if b == nil {
		return nil
	}
	sig := *b
...
func copyTx(b []*types.Transaction) []*types.Transaction {
...
func copyKV(b []*types.KeyValue) []*types.KeyValue {
	kv := make([]*types.KeyValue, 0, len(b))
	for i := 0; i < len(b); i++ {
		kv = append(kv, &types.KeyValue{Key: b[i].Key, Value: b[i].Value})
	}
	return kv
}
