...
func (a *Action) Deposit(payload *et.Deposit) (*types.Receipt, error) {
	var logs []*types.ReceiptLog
	var kvs []*types.KeyValue
	var accountId int32
	var err error

	leaf,err := getLeafByEthAddress(a.localDB, a.fromaddr)
	if leaf == nil {
		AddNewLeaf(a.localDB, a.fromaddr, )
	}


	err = UpdateAccountBalance(a.localDB, accountId, payload.GetTokenId(), int64(payload.GetAmount()), payload.GetChainType())
	if err != nil {
	...
	receiptlog := &types.ReceiptLog{Ty: et.TyDepositLog, Log: types.Encode(accountLog)}
	logs = append(logs, receiptlog)
	receipts := &types.Receipt{Ty: types.ExecOk, KV: kvs, Logs: logs}
	return receipts, nil
}

func (a *Action) Withdraw(payload *et.Withdraw) (*types.Receipt, error) {
	var logs []*types.ReceiptLog
	var kvs []*types.KeyValue
	if payload.GetAccountId() == 0 {
		return nil, errors.New("account not exist")
	}
	err := checkAmount(payload.GetAccountId(), int64(payload.GetAmount()), a.localDB, payload.GetTokenId(), payload.GetChainType())
	if err != nil {
		return nil, errors.Wrapf(err, "db.UpdateAccountBalance")
	}
...
func checkAmount(accountId int32, amount int64, db dbm.KV, tokenId int32, chainType string) error {
